...
module tb_spriteram_dbuf;
	...
	task automatic pulse_copy_start;
		@(posedge clk);
		copy_start = 1;
		@(posedge clk);
		copy_start = 0;
	...
	initial begin
		...
		pulse_copy_start(); // bank0 -> render, bank1 -> write; ctrl_active <= bank0's shadow
		...
		pulse_copy_start(); // bank1 -> render, bank0 -> write; ctrl_active <= bank1's shadow
		@(posedge clk);
