...
module sound_cpu (
	...
	// 4 MHz clock enable for the Z80 core (psikyo.cpp: 4 MHz on every
	// Phase 1 board). Hardwiring CLKEN=1 would run the Z80 at the full
	// 85.909 MHz clk_sys -- ~21x real speed -- so every software-timed
	// behavior (the self-paced boot jingle's delay loops above all) plays
	// garbled. The ROM req/valid handshake below is
...
`endif
	...
	// YM2610 I/O handoff
	output logic         ym_cs,
	output logic [2:0]  ym_addr,   // 3 bits: SH404's OPL4 window is 6 ports
	output logic         ym_rd,
	output logic         ym_wr,
	...
	assign io_ym        = board_sh404   ? ((a[7:0] >= 8'h08) && (a[7:0] <= 8'h0D))
						: board_gunbird ? ((a[7:0] >= 8'h04) && (a[7:0] <= 8'h07))
										: (a[7:0] <= 8'h03);
	// (writes to 0x02/0x03, nop'd in MAME, fall through undecoded)
	...
	assign ym_cs   = io_ym;
	assign ym_addr = a[2:0];   // jt10 uses [1:0]; the OPL4 all three
